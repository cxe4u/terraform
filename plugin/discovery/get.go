...
func (i *ProviderInstaller) Get(provider string, req Constraints) (PluginMeta, tfdiags.Diagnostics, error) {
	...
	// XXX: Temporarily insert warning.
	versionMeta.Warnings = append(versionMeta.Warnings, fmt.Sprintf(`The "%s" provider is deprecated, please use "hashicorp/%s" instead.`, provider, provider))

	for _, warning := range versionMeta.Warnings {
		// TODO: Figure out hostname
		hostname := "registry.terraform.io"
		diag := tfdiags.SimpleWarning(fmt.Sprintf("%s: %s", hostname, warning))
		diags.Append(diag)
		log.Printf("[DEBUG] added diag: %v", diag)
	}
